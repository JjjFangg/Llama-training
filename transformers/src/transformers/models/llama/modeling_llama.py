...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        # todo: keep un_rotated_key_states =================
        ...
        def attn_with_memory(query_states, key_states, value_states, attention_mask,):
            # if (not torch.is_grad_enabled()) and (topk_keys is not None):
            #     print('checkpointing -----------------', query_states.shape, torch.is_grad_enabled(), query_states.requires_grad, )
        ...
        if self.training:  # todo: always checkpointing
            attn_output, attn_weights, past_key_value = torch.utils.checkpoint.checkpoint(
                attn_with_memory, query_states, key_states, value_states, attention_mask, use_reentrant=True,
            )
        else:
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.training:  # todo: always applies gradient checkpointing
